...
class TaskPriorityHistoryDaoTest : KoinTest {
    ...
    fun insertAndGetById_whenEntryIsValid_retrievesCorrectEntry() = runTest {
        ...
        val historyEntry = createHistory(taskId = parentTaskId, priority = PriorityEnum.HIGH, createdAt = 100L)
    ...
    fun getForTask_whenEntriesExist_returnsFilteredAndSortedList() = runTest {
        ...
        historyDao.insert(createHistory(taskId = parentTaskId, priority = PriorityEnum.MEDIUM, createdAt = 200L))
        historyDao.insert(createHistory(taskId = parentTaskId, priority = PriorityEnum.HIGH, createdAt = 100L))
        ...
        historyDao.insert(createHistory(taskId = otherTaskId, priority = PriorityEnum.LOW, createdAt = 400L))
        ...
        assertThat(priorities).containsExactly(PriorityEnum.HIGH, PriorityEnum.MEDIUM, null).inOrder()
    }
    ...
    fun deleteById_shouldRemoveOnlyCorrectEntry() = runTest {
        // GIVEN
        val parentTaskId = taskDao.insert(createTask(id = 1L))
        val idToDelete = historyDao.insert(createHistory(taskId = parentTaskId, priority = PriorityEnum.LOW, createdAt = 100L))
        val idToKeep = historyDao.insert(createHistory(taskId = parentTaskId, priority = PriorityEnum.HIGH, createdAt = 200L))
    ...
    fun insert_withNonExistentParent_throwsConstraintException() = runTest {
        // GIVEN
        val invalidHistoryEntry = createHistory(taskId = 999L, priority = PriorityEnum.MEDIUM, createdAt = 100L)
    ...
    private fun createTask(id: Long, title: String = "Task $id") =
        TaskEntity(taskId = id, title = title, createdAt = 0, updatedAt = 0, isCompleted = false, isArchived = false)
