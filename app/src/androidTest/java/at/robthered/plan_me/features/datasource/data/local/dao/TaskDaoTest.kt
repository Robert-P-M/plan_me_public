...
class TaskDaoTest: KoinTest {
    ...
    fun getRootTaskEntities_withDescSort_returnsCorrectlyFilteredAndSortedList() = runTest {
        ...
        // WHEN
        val rootTasks = taskDao.getRootTaskEntitiesOrderDesc(showCompleted = null, showArchived = null).first()
    ...
    fun getUpcomingTasksForAlarm_returnsCorrectlyFilteredTasks() = runTest {
        ...
        taskDao.insert(createTask(1, "Past", schedule = createSchedule(startDate = 19999, enabled = true)))
        taskDao.insert(createTask(2, "Future Enabled", schedule = createSchedule(startDate = 20001, enabled = true)))
        taskDao.insert(createTask(3, "Future Disabled", schedule = createSchedule(startDate = 20002, enabled = false)))
        taskDao.insert(createTask(4, "Today Enabled", schedule = createSchedule(startDate = 20000, enabled = true)))
    ...
    private fun createTask(
        ...
        return TaskEntity(
            taskId = id, title = title, createdAt = createdAt, updatedAt = createdAt,
            isCompleted = false, isArchived = false, parentTaskId = parentId, sectionId = sectionId, taskSchedule = schedule
        )
    }
