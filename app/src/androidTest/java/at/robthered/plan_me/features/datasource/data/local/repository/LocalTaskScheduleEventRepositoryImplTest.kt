...
class LocalTaskScheduleEventRepositoryImplTest : KoinTest {
    ...
    fun delete_shouldRemoveTheCorrectEntry() = runTest {
        ...
        assertThat(localTaskScheduleEventRepository.getForTask(parentTask.taskId).first()).hasSize(1)

        // WHEN
        val rowsAffected = localTaskScheduleEventRepository.delete(newId)
        val eventsAfterDelete = localTaskScheduleEventRepository.getForTask(parentTask.taskId).first()
    ...
    fun deleteParentTask_whenCascadeIsActive_deletesChildScheduleEvents() = runTest {
        ...
        val event1 = TaskScheduleEventModel(taskId = parentTask.taskId, startDateInEpochDays = 1, createdAt = testTime)
        localTaskScheduleEventRepository.insert(event1)

        val eventsBeforeDelete = localTaskScheduleEventRepository.getForTask(parentTask.taskId).first()
