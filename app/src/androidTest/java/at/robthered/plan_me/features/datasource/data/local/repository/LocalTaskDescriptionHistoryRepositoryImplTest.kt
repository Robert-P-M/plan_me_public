...
class LocalTaskDescriptionHistoryRepositoryImplTest : KoinTest {
    ...
    fun getById_afterInsert_returnsCorrectlyMappedModel() = runTest {
        // GIVEN
        val historyToInsert = TaskDescriptionHistoryModel(taskId = 1L, text = "New Description", createdAt = testTime)
    ...
    fun getForTask_whenMultipleEntriesExist_returnsCompleteList() = runTest {
        // GIVEN
        localTaskDescriptionHistoryRepository.insert(TaskDescriptionHistoryModel(taskId = 1L, text = "Desc 1", createdAt = testTime))
        localTaskDescriptionHistoryRepository.insert(TaskDescriptionHistoryModel(taskId = 1L, text = "Desc 2", createdAt = testTime))
    ...
    fun deleteById_removesOnlySpecifiedEntry() = runTest {
        // GIVEN
        val historyIdToDelete = localTaskDescriptionHistoryRepository.insert(TaskDescriptionHistoryModel(taskId = 1L, text = "History to delete", createdAt = testTime))
        val historyIdToKeep = localTaskDescriptionHistoryRepository.insert(TaskDescriptionHistoryModel(taskId = 1L, text = "History to keep", createdAt = testTime))
    ...
    fun deleteParentTask_whenCascadeIsActive_deletesChildHistory() = runTest {
        // GIVEN
        localTaskDescriptionHistoryRepository.insert(TaskDescriptionHistoryModel(taskId = 1L, text = "History 1", createdAt = testTime))
