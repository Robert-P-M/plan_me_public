...
/**
 * Instrumentation tests for [LocalTaskArchivedHistoryRepositoryImpl].
 *
 * Verifies all CRUD operations and the foreign key cascade behavior using a
 * real in-memory Room database.
 */
@RunWith(AndroidJUnit4::class)
class LocalTaskArchivedHistoryRepositoryImplTest : KoinTest {
    ...
    private val parentTask = TaskEntity(
        taskId = 1L,
        title = "Parent Task for Archived History",
     ...
     * to satisfy the foreign key constraint of the TaskArchivedHistoryEntity.
     */
    @Before
    fun setUp() {
        runBlocking {
    ...
    /**
     * GIVEN a new archived history model for an existing task.
    ...
    fun getById_afterInsert_returnsCorrectlyMappedModel() = runTest {
        // GIVEN
        val historyToInsert = TaskArchivedHistoryModel(taskId = 1L, isArchived = true, createdAt = testTime)

        // WHEN
        val newHistoryId = localTaskArchivedHistoryRepository.insert(historyToInsert)
        val fetchedHistory = localTaskArchivedHistoryRepository.get(newHistoryId).first()

        // THEN
        assertThat(fetchedHistory).isNotNull()
        assertThat(fetchedHistory?.taskArchivedHistoryId).isEqualTo(newHistoryId)
        assertThat(fetchedHistory?.isArchived).isTrue()
    ...
    fun getForTask_whenMultipleEntriesExist_returnsCompleteList() = runTest {
        // GIVEN
        localTaskArchivedHistoryRepository.insert(TaskArchivedHistoryModel(taskId = 1L, isArchived = false, createdAt = testTime))
        localTaskArchivedHistoryRepository.insert(TaskArchivedHistoryModel(taskId = 1L, isArchived = true, createdAt = testTime))

        // WHEN
        val fetchedHistory = localTaskArchivedHistoryRepository.getForTask(1L).first()
    ...
    fun deleteById_removesOnlySpecifiedEntry() = runTest {
        // GIVEN
        val historyIdToDelete = localTaskArchivedHistoryRepository.insert(TaskArchivedHistoryModel(taskId = 1L, isArchived = true, createdAt = testTime))
        val historyIdToKeep = localTaskArchivedHistoryRepository.insert(TaskArchivedHistoryModel(taskId = 1L, isArchived = false, createdAt = testTime))

        // WHEN
        val rowsAffected = localTaskArchivedHistoryRepository.delete(historyIdToDelete)
        val remainingHistory = localTaskArchivedHistoryRepository.getForTask(1L).first()

        // THEN
        assertThat(rowsAffected).isEqualTo(1)
        assertThat(remainingHistory).hasSize(1)
        assertThat(remainingHistory.first().taskArchivedHistoryId).isEqualTo(historyIdToKeep)
    }
    ...
    fun deleteParentTask_whenCascadeIsActive_deletesChildHistory() = runTest {
        // GIVEN
        localTaskArchivedHistoryRepository.insert(TaskArchivedHistoryModel(taskId = 1L, isArchived = true, createdAt = testTime))
        assertThat(localTaskArchivedHistoryRepository.getForTask(1L).first()).hasSize(1)

        // WHEN
        taskDao.delete(taskId = 1L)

        // THEN
        val historyAfterDelete = localTaskArchivedHistoryRepository.getForTask(1L).first()
