...
class TaskArchivedHistoryDaoTest : KoinTest {
    ...
    fun getForTask_whenEntriesExist_returnsFilteredAndSortedList() = runTest {
        // GIVEN
        val parentTaskId = taskDao.insert(createTask(1L))
        historyDao.insert(createHistory(taskId = parentTaskId, isArchived = false, createdAt = 200L))
    ...
    fun insert_withNonExistentParentTask_throwsConstraintException() = runTest {
        ...
        val invalidHistoryEntry = createHistory(taskId = nonExistentTaskId, isArchived = true, createdAt = 100L)
    ...
    // --- Helper Functions ---
    private fun createTask(id: Long) = TaskEntity(taskId = id, title = "Task $id", createdAt = 0, updatedAt = 0, isCompleted = false, isArchived = false)
