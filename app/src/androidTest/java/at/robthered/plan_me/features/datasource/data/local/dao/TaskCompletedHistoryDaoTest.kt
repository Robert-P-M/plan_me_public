...
class TaskCompletedHistoryDaoTest: KoinTest {
    ...
    fun insertAndGetById_whenEntryIsValid_retrievesCorrectEntry() = runTest {
        ...
        val historyEntry = createHistory(taskId = parentTaskId, isCompleted = true, createdAt = 100L)
    ...
    fun getForTask_whenEntriesExist_returnsFilteredAndSortedList() = runTest {
        // GIVEN
        val parentTaskId = taskDao.insert(createTask(id = 1L))
        historyDao.insert(createHistory(taskId = parentTaskId, isCompleted = false, createdAt = 200L))
        historyDao.insert(createHistory(taskId = parentTaskId, isCompleted = true, createdAt = 100L))
    ...
    fun deleteById_shouldRemoveOnlyCorrectEntry() = runTest {
        // GIVEN
        val parentTaskId = taskDao.insert(createTask(id = 1L))
        val idToDelete = historyDao.insert(createHistory(taskId = parentTaskId, isCompleted = true, createdAt = 100L))
        val idToKeep = historyDao.insert(createHistory(taskId = parentTaskId, isCompleted = false, createdAt = 200L))
    ...
    private fun createHistory(taskId: Long, isCompleted: Boolean, createdAt: Long) =
        TaskCompletedHistoryEntity(taskId = taskId, isCompleted = isCompleted, createdAt = createdAt)
}