...
/**
 * Instrumentation tests for [LocalTaskPriorityHistoryRepositoryImpl].
 *
 * Verifies all CRUD operations and the foreign key cascade behavior using a
 * real in-memory Room database.
 */
@RunWith(AndroidJUnit4::class)
class LocalTaskPriorityHistoryRepositoryImplTest : KoinTest {
    ...
    private val parentTask = createTaskEntity(
        id = 1L,
        title = "Parent Task for Priority History",
     ...
     * to satisfy the foreign key constraint of the TaskPriorityHistoryEntity.
     */
    @Before
    fun setUp() {
        runBlocking {
    ...
    /**
     * GIVEN a new priority history model for an existing task.
     * WHEN it is inserted via the repository and then fetched with get().
     * THEN the returned model should match the inserted data.
     */
    @Test
    fun insertAndGetById_returnsCorrectlyMappedModel() = runTest {
        // GIVEN
        val historyToInsert = TaskPriorityHistoryModel(
        ...
        // WHEN
        val newHistoryId = localTaskPriorityHistoryRepository.insert(historyToInsert)
        val fetchedHistory = localTaskPriorityHistoryRepository.get(newHistoryId).first()

        // THEN
        assertThat(fetchedHistory).isNotNull()
        assertThat(fetchedHistory?.taskPriorityId).isEqualTo(newHistoryId)
        assertThat(fetchedHistory?.priorityEnum).isEqualTo(PriorityEnum.HIGH)
    }
    ...
    fun getForTask_whenMultipleEntriesExist_returnsCompleteList() = runTest {
        // GIVEN
        localTaskPriorityHistoryRepository.insert(TaskPriorityHistoryModel(taskId = 1L, priorityEnum = PriorityEnum.LOW, createdAt = testTime))
        localTaskPriorityHistoryRepository.insert(TaskPriorityHistoryModel(taskId = 1L, priorityEnum = PriorityEnum.MEDIUM, createdAt = testTime))

        // WHEN
        val fetchedHistory = localTaskPriorityHistoryRepository.getForTask(1L).first()
    ...
    fun deleteById_removesOnlySpecifiedEntry() = runTest {
        // GIVEN
        val historyIdToDelete = localTaskPriorityHistoryRepository.insert(TaskPriorityHistoryModel(taskId = 1L, priorityEnum = PriorityEnum.HIGH, createdAt = testTime))
        val historyIdToKeep = localTaskPriorityHistoryRepository.insert(TaskPriorityHistoryModel(taskId = 1L, priorityEnum = PriorityEnum.LOW, createdAt = testTime))

        // WHEN
        val rowsAffected = localTaskPriorityHistoryRepository.delete(historyIdToDelete)
        val remainingHistory = localTaskPriorityHistoryRepository.getForTask(1L).first()

        // THEN
        assertThat(rowsAffected).isEqualTo(1)
        assertThat(remainingHistory).hasSize(1)
        assertThat(remainingHistory.first().taskPriorityId).isEqualTo(historyIdToKeep)
    }
    ...
    fun deleteParentTask_whenCascadeIsActive_deletesChildHistory() = runTest {
        // GIVEN
        localTaskPriorityHistoryRepository.insert(TaskPriorityHistoryModel(taskId = 1L, priorityEnum = PriorityEnum.MEDIUM, createdAt = testTime))
        assertThat(localTaskPriorityHistoryRepository.getForTask(1L).first()).hasSize(1)

        // WHEN
        taskDao.delete(taskId = 1L)

        // THEN
        val historyAfterDelete = localTaskPriorityHistoryRepository.getForTask(1L).first()
