...
class GetInboxScreenItemsUseCaseImplTest {

    @MockK
    private lateinit var getInboxScreenTasksUseCase: GetInboxScreenTasksUseCase

    @MockK
    private lateinit var getInboxScreenSectionsUseCase: GetInboxScreenSectionsUseCase

    private lateinit var getInboxScreenItemsUseCase: GetInboxScreenItemsUseCase
    private val testDispatcher = StandardTestDispatcher()

    @BeforeEach
    fun setUp() {
        MockKAnnotations.init(this)
        clearMocks(getInboxScreenTasksUseCase, getInboxScreenSectionsUseCase)
        getInboxScreenItemsUseCase = GetInboxScreenItemsUseCaseImpl(
            getInboxScreenTasksUseCase = getInboxScreenTasksUseCase,
            getInboxScreenSectionsUseCase = getInboxScreenSectionsUseCase,
    ...
    inner class SuccessPath {
        ...
        fun `combines flows successfully`() = runTest(testDispatcher) {
            ...
            getInboxScreenItemsUseCase.invoke(0, null, null, SortDirection.ASC).test {
                ...
                assertThat(finalState.data).containsExactlyElementsIn(expectedCombinedList).inOrder()
