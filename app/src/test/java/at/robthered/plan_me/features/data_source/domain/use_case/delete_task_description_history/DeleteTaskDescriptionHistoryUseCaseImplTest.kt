...
@DisplayName("DeleteTaskDescriptionHistoryUseCaseImpl Tests")
class DeleteTaskDescriptionHistoryUseCaseImplTest: BaseKoinTest() {
    ...
    override val useCaseModule: Module
        get() = module {
            factoryOf(::DeleteTaskDescriptionHistoryUseCaseImpl) {
                bind<DeleteTaskDescriptionHistoryUseCase>()
            }
        }

    private val deleteTaskDescriptionHistoryUseCase: DeleteTaskDescriptionHistoryUseCase by inject()
    ...
    inner class DeletionSucceeds {
        ...
        fun `returns Success when history is deleted`() = runTest {
            // GIVEN
            val historyId = 1L
            coEvery { localTaskDescriptionHistoryRepository.delete(historyId) } returns 1

            // WHEN
            val result = deleteTaskDescriptionHistoryUseCase.invoke(historyId)

            // THEN
            assertThat(result).isInstanceOf(AppResult.Success::class.java)
            coVerify(exactly = 1) { localTaskDescriptionHistoryRepository.delete(historyId) }
        }
    }
    ...
    inner class DeletionFails {
         ...
         * WHEN the use case is invoked.
         * THEN it should return a NO_TASK_DESCRIPTION_HISTORY_FOUND error.
         */
        @Test
        @DisplayName("WHEN rowsAffected is 0 THEN should return NO_TASK_DESCRIPTION_HISTORY_FOUND error")
        fun `returns Error when history is not found`() = runTest {
            // GIVEN
            val historyId = 404L
            coEvery { localTaskDescriptionHistoryRepository.delete(historyId) } returns 0
            ...
            assertThat((result as AppResult.Error).error).isEqualTo(RoomDatabaseError.NO_TASK_DESCRIPTION_HISTORY_FOUND)
            coVerify(exactly = 1) { localTaskDescriptionHistoryRepository.delete(historyId) }
        }
        ...
        fun `returns Error when repository throws exception`() = runTest {
            ...
            coEvery { localTaskDescriptionHistoryRepository.delete(historyId) } throws exception
