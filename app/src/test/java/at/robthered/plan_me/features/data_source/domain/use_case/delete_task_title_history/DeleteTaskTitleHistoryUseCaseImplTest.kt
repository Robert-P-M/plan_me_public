...
@DisplayName("DeleteTaskTitleHistoryUseCaseImpl Tests")
@TestInstance(TestInstance.Lifecycle.PER_CLASS)
class DeleteTaskTitleHistoryUseCaseImplTest {
    ...
    private lateinit var deleteTaskTitleHistoryUseCase: DeleteTaskTitleHistoryUseCase

    @Suppress("UNCHECKED_CAST")
    @BeforeEach
    fun setUp() {
        MockKAnnotations.init(this)
        clearMocks(localTaskTitleHistoryRepository, transactionProvider, safeDatabaseResultCall)
        deleteTaskTitleHistoryUseCase = DeleteTaskTitleHistoryUseCaseImpl(
    ...
    inner class DeletionSucceeds {
        ...
        fun `returns Success when history is deleted`() = runTest {
            // GIVEN
            val historyId = 1L
            coEvery { localTaskTitleHistoryRepository.delete(historyId) } returns 1

            // WHEN
            val result = deleteTaskTitleHistoryUseCase.invoke(historyId)

            // THEN
            assertThat(result).isInstanceOf(AppResult.Success::class.java)
            coVerify(exactly = 1) { localTaskTitleHistoryRepository.delete(historyId) }
        }
    }
    ...
    inner class DeletionFails {
        ...
        fun `returns Error when history is not found`() = runTest {
            // GIVEN
            val historyId = 404L
            coEvery { localTaskTitleHistoryRepository.delete(historyId) } returns 0
        ...
        fun `returns Error when repository throws exception`() = runTest {
            ...
            coEvery { localTaskTitleHistoryRepository.delete(historyId) } throws exception
